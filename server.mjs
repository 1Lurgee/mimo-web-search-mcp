...
// 验证必需的环境变量
if (!MIMO_API_KEY) {
  logger.error("MIMO_API_KEY environment variable is required.");
  process.exit(1);
}
...
if (!validateUrl(MIMO_BASE_URL)) {
  logger.error(`Invalid MIMO_BASE_URL format: ${MIMO_BASE_URL}`);
  process.exit(1);
}
  ...
  async ({ query, max_keyword, limit, force_search, country, region, city }) => {
    ...
    try {
      ...
      for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        try {
          logger.info(`Sending request (attempt ${attempt + 1}/${MAX_RETRIES + 1}): ${query.substring(0, 50)}...`);
          ...
          if (!resp.ok) {
            await resp.text().catch(() => ""); // consume response body
            // 分类处理 HTTP 错误
            if (resp.status === 401 || resp.status === 403) {
              ...
              return {
                content: [
                  {
                    type: "text",
                    text: `MiMo service temporarily unavailable (HTTP ${resp.status}). Please try again later.`,
                  },
                ],
                isError: true,
              };
              ...
              return {
                content: [
                  {
                    type: "text",
                    text: `Request failed with HTTP ${resp.status}. Please check your query parameters.`,
                  },
                ],
                isError: true,
              };
          ...
          logger.info(`Response parsed. Content length: ${content.length}`);
          ...
          return {
            content: [
              {
                type: "text",
                text: `Network error: ${err.message}. Please check your internet connection and try again.`,
              },
            ],
            isError: true,
          };
